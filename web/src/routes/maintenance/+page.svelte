<script lang="ts">
    ...
    // Sort: critical (overdue) first, then by due_at ascending
...
<div class="page-header">
    ...
    <p class="muted">{$_('maintenance.description')}</p>
</div>
...
{:else if alerts.length === 0}
    <EmptyState icon="party-popper" heading={$_('maintenance.empty', { values: { days: withinDays } })} body={$_('maintenance.all_clear')} />
{:else}
...
<style>
    ...
    .alert-links a { color: var(--accent); text-decoration: none; }
