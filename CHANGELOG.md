...
- **Android daily alert notifications.** The app now checks for overdue and due-within-7-days maintenance tasks, chores, and expiry alerts once per day (requires network). A single grouped notification appears in the device notification shade; tapping it opens the Maintenance screen. Android 13+ users are prompted for notification permission on first launch. No Firebase setup required.
