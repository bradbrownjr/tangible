...
Environment variables are prefixed ``TANGIBLE_`` and may also be loaded from
``${TANGIBLE_CONFIG_DIR}/tangible.env`` or ``tangible.yaml``. Secrets accept a
...
def _load_yaml_overlay() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    Top-level scalars become ``TANGIBLE_*`` env-style keys. The optional
    ``oidc_providers:`` list is returned separately because nested config
    can't round-trip cleanly through env vars.
    """
    config_dir = os.environ.get("TANGIBLE_CONFIG_DIR", "/config")
    yaml_path = Path(config_dir) / "tangible.yaml"
    ...
    for k, v in loaded.items():
        ...
        flat[f"TANGIBLE_{k.upper()}" if not k.upper().startswith("TANGIBLE_") else k.upper()] = v
...
class Settings(BaseSettings):
    ...
    session_cookie_name: str = "tangible_session"
    ...
    def _resolve_secrets(self) -> Settings:
        ...
        secret_fields = {
            "db_password": "TANGIBLE_DB_PASSWORD",
            "secret_key": "TANGIBLE_SECRET_KEY",
            "smtp_password": "TANGIBLE_SMTP_PASSWORD",
            "admin_password": "TANGIBLE_ADMIN_PASSWORD",
            "discogs_token": "TANGIBLE_DISCOGS_TOKEN",
            "tmdb_api_key": "TANGIBLE_TMDB_API_KEY",
            "igdb_client_secret": "TANGIBLE_IGDB_CLIENT_SECRET",
            "upcitemdb_key": "TANGIBLE_UPCITEMDB_KEY",
            "google_books_api_key": "TANGIBLE_GOOGLE_BOOKS_API_KEY",
        }
    ...
    def _derive_defaults(self) -> Settings:
        ...
        if self.musicbrainz_user_agent is None:
            object.__setattr__(
                ...
                f"Tangible/{__import__('tangible').__version__} (+{self.public_url})",
            )
    ...
    def resolved_database_url(self) -> str:
        ...
        if self.db_type == "sqlite":
            return f"sqlite:///{self.data_dir / 'tangible.db'}"
        ...
        if driver is None:
            raise ValueError(f"Unsupported TANGIBLE_DB_TYPE: {self.db_type}")

        if not self.db_host:
            raise ValueError("TANGIBLE_DB_HOST is required when TANGIBLE_DATABASE_URL is unset")
