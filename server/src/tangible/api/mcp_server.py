"""Model Context Protocol (MCP) server for Tangible.
...
Auth:
    ...
    identical to the rest of the Tangible API.  Configure your AI client with:
...
from tangible.auth.deps import AuthContext, collection_role
...
class _BearerAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):  # type: ignore[override]
        ...
        settings: Settings = request.app.state.tangible_settings
        ...
        try:
            ...
            request.state.tangible_user_id = user.id
        finally:
            db.close()
...
def _item_to_dict(item: Item, db: DBSession | None = None) -> dict[str, Any]:
    ...
    if item.location_id and db is not None:
        from tangible.models import Location
...
def build_mcp_server(settings: Settings) -> FastMCP:
    ...
    mcp = FastMCP(
        "Tangible",
        instructions=(
            "You have access to the user's Tangible home inventory. "
...
def mcp_app(settings: Settings) -> Starlette:
    ...
    app.state.tangible_settings = settings
