...
def _persist_items(
    ...
    from tangible.api.locations import resolve_or_create as _resolve_loc
...
def export_csv(
    ...
    return PlainTextResponse(
        buf.getvalue(),
        media_type="text/csv",
        headers={"content-disposition": 'attachment; filename="tangible-export.csv"'},
    )
...
def csv_template(category: str = "movies.dvd") -> PlainTextResponse:
    """Download a starter CSV with the column headers Tangible's CSV importer
    ...
    return PlainTextResponse(
        buf.getvalue(),
        media_type="text/csv",
        headers={"content-disposition": f'attachment; filename="tangible-{safe_name}-template.csv"'},
    )
...
def download_backup(
    ...
    return JSONResponse(
        payload,
        headers={"content-disposition": 'attachment; filename="tangible-backup.json"'},
    )
