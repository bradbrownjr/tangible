...
from tangible.auth import service as auth_service
from tangible.auth.deps import AuthContext, require_admin, require_user
from tangible.config import Settings, get_settings
from tangible.db import get_session
from tangible.hardening import DEFAULT_LOGIN_LIMIT, limiter
from tangible.models import APIToken, User
...
def totp_setup(
    ...
    qr_uri = totp.provisioning_uri(name=user.username, issuer_name="Tangible")
...
def export_account(
    ...
    The JSON backup uses the same format as ``tangible backup`` CLI and can be
    restored via ``tangible restore`` or the web import wizard.
    """
    from tangible.importers.json_backup import export_user
    ...
    with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            "README.txt",
            (
                "This archive was exported from Tangible.\n\n"
                "backup.json contains all collections, items, tags, contacts, and loans.\n"
                "Restore it via: tangible restore backup.json\n"
...
def delete_account(
    ...
    """
    from tangible.models.collection import Collection, CollectionMembership
