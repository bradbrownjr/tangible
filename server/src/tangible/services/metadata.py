...
USER_AGENT = "Tangible/1.0 (+https://github.com/bradbrownjr/tangible)"
...
def discover_plugins() -> None:
    ...
    URL adapters (``tangible.scraper_adapter`` group) are prepended before the
    OpenGraph fallback. Barcode adapters (``tangible.barcode_adapter`` group) are
    prepended before the built-in barcode adapters.

    Third-party packages declare adapters in their ``pyproject.toml``::

        [project.entry-points."tangible.scraper_adapter"]
        myadapter = "mypackage.adapters:MyUrlAdapterClass"

        [project.entry-points."tangible.barcode_adapter"]
    ...
    for ep in importlib.metadata.entry_points(group="tangible.scraper_adapter"):
        try:
            cls = ep.load()
    ...
    for ep in importlib.metadata.entry_points(group="tangible.barcode_adapter"):
        try:
            cls = ep.load()
