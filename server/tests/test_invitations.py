...
def test_create_and_accept_invitation(client) -> None:
    ...
    r = client.post(
        f"/api/collections/{cid}/invitations",
    ...
    # Bob accepts.
    _login(client, "bob")
    r = client.post(f"/api/invitations/{token}/accept")
...
def test_invitation_token_single_use(client) -> None:
    ...
    token = client.post(
        f"/api/collections/{cid}/invitations", json={"role": "viewer"}
    ).json()["token"]
    _logout(client)

    _login(client, "bob")
    assert client.post(f"/api/invitations/{token}/accept").status_code == 204
...
def test_unknown_token_404(client) -> None:
    r = client.get("/api/invitations/not-a-token")
...
def test_revoke_invitation(client) -> None:
    ...
    _login(client, "alice")
    cid = client.post("/api/collections", json={"name": "X"}).json()["id"]
    r = client.post(f"/api/collections/{cid}/invitations", json={"role": "viewer"})
    inv_id = r.json()["id"]
    token = r.json()["token"]
    assert client.delete(
        f"/api/collections/{cid}/invitations/{inv_id}"
    ).status_code == 204
    _logout(client)
    _login(client, "bob")
