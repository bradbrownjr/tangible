...
def test_add_remove_member_flow(client) -> None:
    ...
    r = client.patch(
        f"/collections/{cid}/members/{member_id}",
        json={"role": "viewer"},
    )
    ...
    assert client.delete(f"/collections/{cid}/members/{member_id}").status_code == 204
    _logout(client)

    # Bob has lost access.
    _login(client, "bob")
    assert client.get(f"/collections/{cid}/members").status_code == 403
...
def test_editor_can_modify_items_viewer_cannot(client) -> None:
    ...
    _login(client, "bob")
    assert client.patch(f"/items/{iid}", json={"condition": "Mint"}).status_code == 200
    _logout(client)

    _login(client, "carol")
    assert client.get(f"/items/{iid}").status_code == 200
    assert client.patch(f"/items/{iid}", json={"condition": "Poor"}).status_code == 403
