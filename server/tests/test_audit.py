...
def test_audit_records_member_add(client, db) -> None:
    ...
    r = client.get(f"/api/audit?collection_id={cid}")
...
def test_audit_records_share_link_lifecycle(client) -> None:
    ...
    cid = client.post("/api/collections", json={"name": "X"}).json()["id"]
    link_id = client.post(
        f"/api/collections/{cid}/share-links", json={"label": "ph"}
    ).json()["id"]
    client.delete(f"/api/collections/{cid}/share-links/{link_id}")
    r = client.get(f"/api/audit?collection_id={cid}")
    actions = [e["action"] for e in r.json()]
...
def test_audit_records_invitation_lifecycle(client) -> None:
    ...
    _login(client, "bob")
    client.post(f"/api/invitations/{token}/accept")
    client.post("/api/auth/logout")
    _login(client, "alice")
    r = client.get(f"/api/audit?collection_id={cid}")
    actions = [e["action"] for e in r.json()]
...
def test_audit_global_admin_ok(client, db) -> None:
    ...
    _login(client, "alice")
    cid = client.post("/api/collections", json={"name": "X"}).json()["id"]
    client.post(f"/api/collections/{cid}/share-links", json={})
    r = client.get("/api/audit")
    assert r.status_code == 200
