...
def test_upload_and_download_pdf(client) -> None:
    ...
    r = client.get(f"/items/{iid}/documents")
...
def test_expiring_dashboard(client) -> None:
    ...
    # Item expiring soon
    client.patch(f"/items/{iid}", json={"expires_at": soon})
    ...
    r = client.get("/expiring?within_days=30")
    assert r.status_code == 200
    rows = r.json()
    ...
    r = client.get("/expiring?within_days=500")
    rows = r.json()
