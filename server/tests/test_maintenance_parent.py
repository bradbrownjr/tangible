...
def test_maintenance_task_lifecycle(client) -> None:
    ...
    iid = client.post(
        "/api/items", json={"collection_id": cid, "category": "tools.hand", "title": "Mower"}
    ).json()["id"]
    ...
    r = client.post(f"/api/maintenance/{task['id']}/complete")
    assert r.status_code == 200
    assert r.json()["last_completed_at"] is not None

    r = client.get(f"/api/items/{iid}/maintenance")
    assert len(r.json()) == 1

    r = client.get("/api/maintenance?within_days=365")
    assert any(t["id"] == task["id"] for t in r.json())

    r = client.delete(f"/api/maintenance/{task['id']}")
...
def test_parent_child_items(client) -> None:
    ...
    parent = client.post(
        "/api/items", json={"collection_id": cid, "category": "other.generic", "title": "Camera kit"}
    ).json()
    child = client.post(
        "/api/items",
        json={
    ...
    r = client.get(f"/api/items/{parent['id']}/children")
    assert r.status_code == 200
...
def test_parent_self_rejected(client) -> None:
    ...
    r = client.patch(f"/api/items/{iid}", json={"parent_id": iid})
...
def test_parent_cycle_rejected(client) -> None:
    ...
    a = client.post(
        "/api/items", json={"collection_id": cid, "category": "other.generic", "title": "A"}
    ).json()["id"]
    b = client.post(
        "/api/items",
    ...
    r = client.patch(f"/api/items/{a}", json={"parent_id": b})
    assert r.status_code == 422


def test_parent_must_be_same_collection(client) -> None:
    cid1 = _setup(client, "xcuser")
    cid2 = client.post("/api/collections", json={"name": "Other"}).json()["id"]
    a = client.post(
        "/api/items", json={"collection_id": cid1, "category": "other.generic", "title": "A"}
    ).json()["id"]
    b = client.post(
        "/api/items", json={"collection_id": cid2, "category": "other.generic", "title": "B"}
    ).json()["id"]
    r = client.patch(f"/api/items/{b}", json={"parent_id": a})
    assert r.status_code == 422
